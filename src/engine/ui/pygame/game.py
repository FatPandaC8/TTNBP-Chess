...
class Game:
        ...
        while self.running:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if not self._try_forward_to_agent(event):
                        self.running = False
                else:
    ...
    def _handle_event(self, event: pygame.event.Event) -> None:
        if self._state == "menu":
            ...
            self._try_forward_to_agent(event)
    ...
    def _try_forward_to_agent(self, event: pygame.event.Event) -> bool:
        if self._state != "game":
            return False
        ...
        was_promoting = agent.promotion_picker is not None

        promo_move = agent.handle_event(event)
        if promo_move is not None:
            self._apply_move(promo_move)
            return True

        return was_promoting or agent.promotion_picker is not None
    ...
    def _draw_game(self) -> None:
        ...
        promo_picker = getattr(agent, "promotion_picker", None)

        hover_sq = None
        if isinstance(agent, HumanAgent) and agent.input_handler and promo_picker is None:
    ...
    def _draw_side_panel(self) -> None:
        ...
        pygame.draw.rect(self.screen, BG_PANEL, (panel_x, panel_y, panel_w, panel_h), border_radius=6)
    ...
    def _draw_game_over_overlay(self) -> None:
        ...
        self.screen.blit(sub, (bx + (BOARD_PX - sub.get_width()) // 2, ty + title.get_height() + 8))
